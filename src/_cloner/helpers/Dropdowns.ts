...
const dropdownCustomer = (data: any) => {
    return (
        data &&
        data?.map((obj: { firstName: string; lastName: string, id: string, customerValidityColorCode: string, customerCode: number }): any => {
            const { firstName, lastName, id, customerValidityColorCode, customerCode } = obj;
            return { value: id, label: `${customerCode} - ${firstName} ${lastName}` , customerValidityColorCode: customerValidityColorCode };
        })
    );
};
