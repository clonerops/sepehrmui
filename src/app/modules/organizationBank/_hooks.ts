...
const useGetOrganizationBankList = () => {
    return useQuery(['organizationBank'], () => 
        api.getOrganizationBankList());
};
const usePostOrganizationBank = () => {
    return useMutation((formdata: IOrganizationBank) => 
        api.postOrganizationBank(formdata));
};
const usePutOrganizationBank = () => {
    return useMutation((formdata: IOrganizationBank) => 
        api.putOrganizationBank(formdata));
};

const useGetOrganizationBankById = () => {
    return useMutation((id: number) => 
        api.getOrganizationBankById(id))
}

const useDeleteOrganizationBank = () => {
    return useMutation((id: string) => 
        api.deleteOrganizationBank(id));
};
